...
LoadBalancedCluster.prototype.addNode = function( callback ) {
	...
	this._getLeastPopulatedZone( function( error, zone ) {
		...
		{
			self._launchNewNodeInZone( zone, function( error, instances ) {
				if( error )
					callback( error );
				else
				{
			...
			if( balanced.length > 1 )
			{
				ec2.getRunningInstances( self.config.region, function( error, instances ) {
					...
					self._getMostPopulatedZone( function( error, zone ) {
						...
						self.logger.log( 'elastic (' + self.config.name + '): terminating node: ' + firstInstanceInZone.instance + ' in ' + zone );
						ec2.removeInstanceFromLoadBalancer( self.config.region, firstInstanceInZone.instance, self.config.name, function( error, result ) {
							if( error )
								callback( error );
							else
							{
								self._optionallyNotifyInstanceOfShutdown( firstInstanceInZone.instance, instances, function( error ) {
									if( error )
										self.logger.log( 'elastic (' + self.config.name + '): error notifying instance of shutdown: ' + error );
									ec2.terminateInstance( self.config.region, firstInstanceInZone.instance, function( error, result ) {
										callback( error );
									} );
								} );
							}
