...
LoadBalancedCluster.prototype.setEc2Client = function( client ) {
	this.ec2 = client;
}
LoadBalancedCluster.prototype.getEc2Client = function() {
	return this.ec2;
}
							...
							else
							{
								...
								async.forEachSeries( instances, 
									function( instance, done ) {
										...
										{
											self.ec2.getAverageCPUUtilization( 
...
LoadBalancedCluster.prototype._checkSecurityGroupExists = function( callback ) {
	var self = this;
	self.ec2.getSecurityGroups( this.config.region, function( error, groups ) {
		if( error )
			callback( error, null );
		else
		{
...
LoadBalancedCluster.prototype._createSecurityGroup = function( callback ) {
	...
	var self = this;
	self.ec2.createSecurityGroup( this.config.region, this.config.securityGroup, function( error, result ) {
		if( error )
			callback( error );
		else
			async.forEachSeries( self.config.firewallAllow, 
				function( rule, done ) {
					self.ec2.addFirewallAllow( self.config.region, self.config.securityGroup, rule, done );
				},
				callback
			);
	});
...
LoadBalancedCluster.prototype._checkLoadBalancerExists = function( callback ) {
	var self = this;
	self.ec2.getElasticLoadBalancers( this.config.region, function( error, balancers ) {
		if( error )
			callback( error, null );
		else
		{
...
LoadBalancedCluster.prototype._createLoadBalancer = function( callback ) {
	...
	self.ec2.createElasticLoadBalancer( 
		this.config.name, 
		this.config.region, 
		this.config.zones, 
		[ {
...
LoadBalancedCluster.prototype._checkFirstNodeExists = function( callback ) {
	var self = this;
	self.ec2.getRunningInstances( this.config.region, function( error, instances ) {
		if( error )
			callback( error );
		else
		{
...
LoadBalancedCluster.prototype._launchNewNode = function( callback ) {
	var self = this;
	self.ec2.launchInstance( this.config.region, this.config.ami, 
...
LoadBalancedCluster.prototype._launchNewNodeInZone = function( zone, callback ) {
	...
	self.ec2.launchInstanceInAvailabilityZone( this.config.region, this.config.ami, 
	...
	this._getLeastPopulatedZone( function( error, zone ) {
		...
		else
		{
			self._launchNewNodeInZone( zone, function( error, instance ) {
				...
				{
					self.ec2.attachInstanceToLoadBalancer( 
						...
						function( error, result ) {
							...
							{
								self.ec2.addLoadBalancerAvailabilityZone( self.config.region, self.config.name, zone, function( error, activeZones ) {
									callback( error );
								});
							}
						}
...
LoadBalancedCluster.prototype.getNumberOfRunningNodes = function( callback ) {
	self.ec2.getBalancedInstances( this.config.region, this.config.name, function( error, balanced ) {
		if( error )
			callback( error, -1 );
		else
...
LoadBalancedCluster.prototype.getNumberOfNodesPerZone = function( callback ) {
	...
	self.ec2.getBalancedInstances( this.config.region, this.config.name, function( error, balanced ) {
		...
		{
			self.ec2.getRunningInstances( self.config.region, function( error, instances ) {
				if( error )
					callback( error, null );
				else
				{
...
LoadBalancedCluster.prototype.removeNode = function( callback ) {
	...
	self.ec2.getBalancedInstances( self.config.region, self.config.name, function( error, balanced ) {
		if( error )
			callback( error );
		else
		{
...
LoadBalancedCluster.prototype._terminateInstance = function( instance, runningInstances, callback ) {
	...
	self.logger.log( 'elastic (' + self.config.name + '): terminating node: ' + instance.instance );
		self.ec2.removeInstanceFromLoadBalancer( self.config.region, instance.instance, self.config.name, function( error, result ) {
			if( error )
				callback( error );
			else
			{
				self._optionallyNotifyInstanceOfShutdown( instance.instance, runningInstances, function( error ) {
					...
					self.ec2.terminateInstance( self.config.region, instance.instance, function( error, result ) {
								...
								else
								{
									...
									self.ec2.attachInstanceToLoadBalancer( 
										...
										function( error, result ) {
											...
											{
												self.ec2.addLoadBalancerAvailabilityZone( self.config.region, self.config.name, instance.zone, function( error, activeZones ) {
													callback( error );
												});
