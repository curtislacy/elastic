...
ProcessingCluster.prototype._getLowUtilizationSystem = function( callback ) {
	...
	this._findLowUtilizationSystem( function( error, system ) {
		...
		else
		{
			...
			{
				// TODO: Check to see if there's already a system being created.
				// Don't make a new one if so.
...
ProcessingCluster.prototype._createNewSystem = function( callback ) {
	...
	*/
	...
	if( lowestZone )
	{
		ec2.launchInstanceInAvailabilityZone( 
			this.config.region, 
			this.config.ami,
			this.config.keypair,
			this.config.type,
			...
			function( error, launched ) {
				...
				{
					this.idleSystems[ launched.instance ] = {
						system: launched,
						util: 0,
						idleSince: ( new Date() ).getTime()
					};
	...
	{
		callback( new Error( 'Could not determine most lightly used availablity zone.' ), null );
	}
}
